...
public class HotelServiceImp implements HotelService {
	...
	/**
	 * Returns all Hotel objects in the database that are available in between user
	 * specified dates
	 *
	 * @param dateFrom
	 * @param dateTo
	...
	public IdEntity saveHotel(@Valid Hotel hotel) {
		// If dates are empty strings make them null values so that they can be accepted
		// by the database
	...
	public SuccessEntity deleteHotel(Integer id) {
		validateHotelExistenceById(id);
		if (reservationRepository.findAll().stream().anyMatch(reservations -> reservations.getHotelId().equals(id))) {
	...
	public void doesReservationOverlap(Hotel hotel) {
		...
		List<Reservation> matchingReservationList = reservationRepository.findAll().stream().filter(reservations -> {
			if (reservations.getHotelId() == hotelId) {
				try {
					// Checks to see if the user dates are null, if so throw an error as it
					// conflicts with a reservation
					...
					// should return 1 or 0 if there is no overlap, should return -1 if there is an
					// overlap
